...
*/        
		...
		valueTile(	"monthtripsAvgAverageKmpl", "device.monthlyTripsAvgAverageKmpl",width: 2, height: 2,canChangeIcon: false,
 					decoration: "flat"
				) 
		{
			state("default",
		...
		valueTile(	"monthtripsAvgDistanceM", "device.monthlyTripsAvgDistanceM",width: 2, height:2,canChangeIcon: false,
 					decoration: "flat"
				) 
		{
			state("default",
		...
		valueTile(	"monthtripsAvgDurationS", "device.monthlyTripsAvgDurationS", unit:'min.', width: 2, height: 2,canChangeIcon: false,
  					decoration: "flat"
				)
		{
			state("default",
		...
		valueTile(	"monthtotalDistanceM", "device.monthlyTotalDistanceM",width: 2, height: 2, canChangeIcon: false,
  					decoration: "flat"
				)
		{
			state("default",
		...
		valueTile(	"monthtotalFuelVolumeL", "device.monthlyTotalFuelVolumeL",width: 2, height: 2,canChangeIcon: false,
  					decoration: "flat"
				)
		{
			state("default",
		...
		valueTile(	"monthtotalCostFuel", "device.monthlyTotalCostUsd",width: 2, height: 2,canChangeIcon: false,
  					decoration: "flat",
				)
		{
			state("default",
		...
		valueTile(	"monthtotalNbTrips", "device.monthlyTotalNbTrips",width: 2, height: 2,canChangeIcon: false,
  					decoration: "flat"
				)
		{
			state("default",
		...
		valueTile(	"monthtripsAvgScoreSpeeding", "device.monthlyTripsAvgScoreSpeeding",width: 2, height: 2,canChangeIcon: false,
  					decoration: "flat"
				)
		{
			state("default",
		...
		valueTile(	"monthtripsAvgScoreEvents", "device.monthlyTripsAvgScoreEvents",width: 2, height: 2,canChangeIcon: false,
  					decoration: "flat"
				)
		{
			state("default",
		...
		details(["main_multi", "make", "model",  "refresh",  "submodel", "year", "present", "switch", "totalNbTrips", 
			...
			"monthtotalFuelVolumeL","monthtotalCostFuel", "monthtotalNbTrips","monthtripsAvgScoreSpeeding", "monthtripsAvgScoreEvents",
			"graphHTML"                        
		])        
...
def updated() {
	state?.totalWeeklyDistanceTable = null
	state?.totalMonthlyDistanceTable = null
	state?.totalWeeklyCostTable = null
	state?.totalMonthlyCostTable = null
	state?.dailyDistanceTable = null
	state?.dailyCostTable = null
	state?.scale=getTemperatureScale()    
	log.debug ("updated>$device.displayName updated with settings: ${settings.inspect()} and state variables= ${state.inspect()}")
...
void poll() {
	...
	if ((state?.lastGeneratedStatsDate != dateInLocalTime) || (state?.localFuelCostPerVolUnit != settings.localFuelCostPerVolUnit)) {
		...
		generateMonthlyTripStats(vehicleId) 
	...
	refresh_automatic(vehicleId)    

}

private def refresh_automatic(vehicleId) {
	String dateInLocalTime = new Date().format("yyyy-MM-dd", location.timeZone)
	def vehicleMake
	def vehicleModel
	def vehicleSubmodel
	...
	if (settings.trace) {
		log.debug("poll>about to call getTrips for today at midnight ${dateAtMidnight} in local time")      
	}
...
//	Generate Events since last poll() call only
	...
	if (settings.trace) {
		log.debug "poll>about to call retrieveDataForGraph()"
	}
   	retrieveDataForGraph()     
...
*/    
	...
	if (state.exceptionCount >= MAX_EXCEPTION_COUNT) {
		...
		log.warn ("api>error: found a high number of exceptions (${state.exceptionCount}), last exceptionCheck=${exceptionCheck}, about to reset counter")    
...
private def doRequest(uri, args, type, success) {
	...
	try {
		...
		state?.exceptionCount=0 // reset the counter when success        
...
//	Generate stats for yesterday
	...
	if (settings.trace) {
		log.debug("generateYesterdayTripStats>date at Midnight= ${dateAtMidnight}")
	}
	...
	getTrips(vehicleId,"", startDate,endDate)
...
void generateWeeklyTripStats(vehicleId) {
	...
	String dateAtMidnight = dateInLocalTime + " 00:00 " + timezone    
	if (settings.trace) {
		log.debug("generateWeeklyTripStats>date at Midnight= ${dateAtMidnight}")
	}
	Date endDate = formatDate(dateAtMidnight) 
...
//	Generate stats for the past month
	...
	Date endDate = new Date()
	if (settings.trace) {
		log.debug("generateMonthlyTripStats>startDate in UTC =${oneMonthAgo.format("yyyy-MM-dd HH:mm:ss", TimeZone.getTimeZone("UTC"))}" +
			",endDate in UTC =${endDate.format("yyyy-MM-dd HH:mm:ss", TimeZone.getTimeZone("UTC"))}")        	
		log.debug("generateMonthlyTripStats>about to call getTripsAsync...")
	}    
	getTripsAsync(vehicleId,oneMonthAgo,endDate, 'getAsyncMonthlyStatsResponse')
}

private def generateMonthlyStatsEvents() {
...
void getTripsAsync(vehicleId,startDateTime,endDateTime, getResponseFunction) {
...
//	Set the limit to 250 trips per page.    
	def bodyReq="?vehicle=" + vehicleId + "&limit=${AUTOMATIC_MAX_TRIP_LIMIT}"
	...
	if (settings.trace) {
		log.debug "getTripsAsync>bodyReq=${bodyReq}" 
	}				                
		    
	def params = [
		uri: "${URI_ROOT}/trip/${bodyReq}",
		headers: [
			'Authorization': "${data.auth.token_type} ${data.auth.access_token}",
			'Content-Type': "application/json",
			'charset': "UTF-8",
	...
	asynchttp_v1.get("${getResponseFunction}", params)
}

def getAsyncMonthlyStatsResponse(response, data) {	
	def TOKEN_EXPIRED=401
	  
	if (response.hasError()) {
		log.error("getAsyncMonthlyStatsResponse>automatic response error: $response.errorMessage")
		state?.exceptionCount=state?.exceptionCount +1        
	}        
	if (response?.status == TOKEN_EXPIRED) { // token is expired
		log.warn("getAsyncMonthlyStatsResponse>Automatic's Access token has expired, need to re-login at automatic using MyAutomaticServiceMgr...")
	} else {
		try {
			...
			if (responseValues) {
				...
				if (settings.trace) {
					log.debug "getAsyncMonthlyStatsResponse>response.json metadata=${response.json?._metadata}" 
					if (response.json?._metadata) {
						log.debug "getAsyncMonthlyStatsResponse>responseValues for 1st trip id=${responseValues?.results[0]?.id}" 
					}                
				}				 
				processTripsData(responseValues)
				generateMonthlyStatsEvents()                    
				state?.exceptionCount=0                 
			}                
                
		} catch (e) {
			log.error("getAsyncMonthlyStatsResponse>Automatic - error parsing json from response: $e")
			return            
		}
	}        
...
void getTrips(vehicleId,tripId,startDateTime,endDateTime, eventTimestamp=null,postData=false) {
	def AUTOMATIC_SUCCESS=200
	...
	while ((statusCode!= AUTOMATIC_SUCCESS) && (j++ <2)) { // retries once if api call fails
		api('trips', bodyReq) {resp->
			...
			if (statusCode == AUTOMATIC_SUCCESS) {
				if (settings.trace) {
					log.debug "getTrips>resp.data.results=${resp?.data}" 
					log.debug "getTrips>resp.data.metadata=${resp.data?._metadata}" 
				}	
				...
				processTripsData(null,eventTimestamp,postData)               	
				sendEvent name: "verboseTrace", displayed: ((settings.trace)?: false), value:
...
private def processTripsData(refreshValues,eventTimestamp=null,postData=false) {
	...
	if (settings.trace) {
    	if (refreshValues) {
			log.debug ("processTripsData>refreshValues._metadata=${refreshValues?._metadata}")
		}
	}        
	if ((refreshValues) && (refreshValues?._metadata==null)) {
		// just one trip was found                
		data?.trips?.results[0]=refreshValues
		data?.trips?._metadata?.count=1   
		if (settings.trace) {
			log.debug "processTripsData>just 1 result was found for a given trip" 
		}        
	} else if ((refreshValues?.results != null) && (refreshValues?._metadata != null)) {
		data?.trips=refreshValues
		if (settings.trace) {
			log.debug "processTripsData>data?.trips._metadata now = ${data?.trips?._metadata}" 
		}        
	}
	if ((data?.trips?.results != null && data?.trips?.results != [])) {
		for (i in 0..data?.trips._metadata.count -1 ) {
			...
			if (postData) {
				...
				if (data.trips._metadata.count <= MAX_TRIP_DATA) {
					...
					log.error "processTripsData>vehicleId=${vehicleId},too much data to post, count=${data.trips._metadata.count} trips, no posting it"
				}                        
			}
			...
			if (settings.trace) {
				log.debug "processTripsData>startedAt=${startedAt},endedAt=${endedAt}," +
				...
				sendEvent name: "verboseTrace", value: "processTripsData>tripId=${id},startedAt=${startedAt},endedAt=${endedAt}," +
					"distanceM=${distanceM},durationS=${durationS},vehicleURL=${vehicleURL},startLocation=${startLocation},startAddress=${startAddress}," +
					"endLocation=${endLocation},endAddress=${endAddress},startTimezone=${startTimezone},endTimezone=${endTimezone}," +       		
					"averageKmpl=${averageKmpl},fuelVolumeL=${fuelVolumeL},fuelCostUsd=${fuelCostUsd}," +
					"hardAccels=${hardAccels},hardBrakes=${hardBrakes},scoreSpeeding=${scoreSpeeding},scoreEvents=${scoreEvents}\n" +
	...
	if (data?.trips?._metadata?.next) {
		if (settings.trace) {
			log.debug "processTripsData>vehicleId=${vehicleId},nextPage=${data.trips._metadata.next}"
		}                
		...
		if (settings.trace) {
			log.trace "processTripsData>nbTrips=${nbTrips}, totalDistanceM=${totalDistanceM},tripsAvgDistanceM=${tripsAvgDistanceM}" 
			log.trace "processTripsData>nbTrips=${nbTrips}, totalDurationS=${totalDurationS},tripsAvgDurationS=${tripsAvgDurationS}" 
			log.trace "processTripsData>nbTrips=${nbTrips}, totalCostUsd=${totalCostUsd},tripsAvgCostUsd=${tripsAvgCostUsd}" 
			log.trace "processTripsData>nbTrips=${nbTrips}, totalFuelVolumeL=${totalFuelVolumeL},tripsAvgFuelVolumeL=${tripsAvgFuelVolumeL}" 
			log.trace "processTripsData>nbTrips=${nbTrips}, totalAverageKmpl=${totalAverageKmpl},tripsAvgAverageKmpl=${tripsAvgAverageKmpl}" 
			log.trace "processTripsData>nbTrips=${nbTrips}, totalScoreSpeeding=${totalScoreSpeeding},tripsAvgScoreSpeeding=${tripsAvgScoreSpeeding}" 
			log.trace "processTripsData>nbTrips=${nbTrips}, totalScoreEvents=${totalScoreEvents},tripsAvgScoreEvents=${tripsAvgScoreEvents}" 
		}					                
...
private def refresh_tokens() {
	...
	def authexptime = new Date((now() + (resp.data.expires_in * 365 * 24 * 60 * 60 * 1000))).getTime() // token expires in a year
...
void setAuthTokens() {
	...
	def authexptime = new Date((now() + (data.auth.expires_in * 365 * 24* 60 * 60 * 1000))).getTime()
	data.auth.authexptime = authexptime
	if (settings.trace) {
...
*/	
	...
	data?.auth = settings  
	data?.auth?.vehicleId = device_automatic_id 
	...
	refresh_automatic(device_automatic_id)
}
...
def retrieveDataForGraph() {
	...
	def MIN_DEVIATION_DIST=(state?.scale=='C')?8:5
	def MIN_DEVIATION_COST=0.5
    
	if (settings.trace) {    
		log.debug ("today at Midnight in local time= ${todayAtMidnight}")
	}        
	def dailyDistanceTable = []
	def dailyCostTable = []
	def dailyDistanceData = device.statesSince("totalDistanceM", startOfWeek, [max:100])
	def dailyCostData = device.statesSince("totalCostUsd", startOfWeek, [max:100])
/*    
	dailyDistanceData.reverse().each() {
		dailyDistanceTable.add([it.date.getTime(),it.floatValue])
	}
	dailyCostData.reverse().each() {
		dailyCostTable.add([it.date.getTime(),it.floatValue])
	}
*/
	def previousValue=null
	int maxInd=dailyDistanceData?.size()-1    
	for (int i=maxInd; (i>=0);i--) {
    	if (i !=maxInd) previousValue = dailyDistanceData[i+1]?.floatValue
		// filter some values        
		if ((i==0) || (i==maxInd) || ((dailyDistanceData[i]?.floatValue <= (previousValue - MIN_DEVIATION_DIST)) || (dailyDistanceData[i]?.floatValue >= (previousValue + MIN_DEVIATION_DIST)) )) {
 			dailyDistanceTable.add([dailyDistanceData[i].date.getTime(),dailyDistanceData[i].floatValue])
		}            
	}
	previousValue=null
	maxInd=dailyCostData?.size()-1    
	for (int i=maxInd; (i>=0);i--) {
    	if (i !=maxInd) previousValue = dailyCostData[i+1]?.floatValue
		// filter some values        
		if ((i==0) || (i==maxInd) || ((dailyCostData[i]?.floatValue <= (previousValue - MIN_DEVIATION_COST)) || (dailyCostData[i]?.floatValue >= (previousValue + MIN_DEVIATION_COST)) )) {
 			dailyCostTable.add([dailyCostData[i].date.getTime(),dailyCostData[i].floatValue])
		}            
	}


	if (dailyDistanceTable == []) { // if the totalDistanceM has not changed for a week
		float currentDistanceM=device.currentValue("totalDistanceM")?.toFloat()
		dailyDistanceTable.add([startOfWeek.getTime(),currentDistanceM])		        
		dailyDistanceTable.add([todayDate.getTime(),currentDistanceM])		        
	} else {
		float currentDistanceM=device.currentValue("totalDistanceM")?.toFloat()
		dailyDistanceTable.add([todayDate.getTime(),currentDistanceM])		        
	}    
	if (dailyCostTable == []) {  // if the tripsAvgAverageKmpl has not changed for a week
		float currentCost=device.currentValue("totalCostUsd")?.toFloat()
		dailyCostTable.add([startOfWeek.getTime(),currentCost])		        
		dailyCostTable.add([todayDate.getTime(),currentCost])		        
	} else {
		float currentCost=device.currentValue("totalCostUsd")?.toFloat()
		dailyCostTable.add([todayDate.getTime(),currentCost])		        
	}    
	...
	def totalWeeklyDistanceTable = (state?.totalWeeklyDistanceTable)?:null  
	def totalMonthlyDistanceTable = (state?.totalMonthlyDistanceTable)?:null  
	def totalWeeklyCostTable = (state?.totalWeeklyCostTable)?:null 
	def totalMonthlyCostTable = (state?.totalMonthlyCostTable)?:null
	
	if (totalWeeklyDistanceTable == null || totalMonthlyDistanceTable == null || totalWeeklyCostTable == null || totalMonthlyCostTable == null) {
		if (settings.trace) {    
			log.debug("Querying ST for stats dataÖ")
		}            
		totalWeeklyDistanceTable = []
		totalMonthlyDistanceTable = []
		totalWeeklyCostTable = []
		totalMonthlyCostTable = []
		def totalWeeklyDistanceData = device.statesSince("weeklyTotalDistanceM", startOfWeek, [max:50])
		def totalMonthlyDistanceData = device.statesSince("monthlyTotalDistanceM", startOfWeek, [max:50])
		def totalWeeklyCostData = device.statesSince("weeklyTotalCostUsd", startOfWeek, [max:50])
		def totalMonthlyCostData = device.statesSince("monthlyTotalCostUsd", startOfWeek, [max:50])

		totalWeeklyDistanceData.reverse().each() {
			totalWeeklyDistanceTable.add([it.date.getTime(),it.floatValue])
		}
		totalMonthlyDistanceData.reverse().each() {
			totalMonthlyDistanceTable.add([it.date.getTime(),it.floatValue])
		}
		totalWeeklyCostData.reverse().each() {
			totalWeeklyCostTable.add([it.date.getTime(),it.floatValue])
		}
		totalMonthlyCostData.reverse().each() {
			totalMonthlyCostTable.add([it.date.getTime(),it.floatValue])
		}
		if (totalMonthlyDistanceData == []) {  // if the table is empty, just add the current distance
			float currentDistance=device.currentValue("totalDistanceM")?.toFloat()
			totalMonthlyDistanceTable.add([todayDate.getTime(),currentDistance])		        
		}        
		if (totalWeeklyDistanceTable == []) {  // if the table is empty, just add the current distance
			float currentDistance=device.currentValue("totalDistanceM")?.toFloat()
			totalMonthlyDistanceTable.add([todayDate.getTime(),currentDistance])		        
		}        
		if (totalMonthlyCostTable == []) {  // if the table is empty, just add the current cost
			float currentCost=device.currentValue("totalCostUsd")?.toFloat()
			totalMonthlyCostTable.add([todayDate.getTime(),currentCost])		        
		}        
		if (totalWeeklyCostTable == []) {  // if the table is empty, just add the current cost
			float currentCost=device.currentValue("totalCostUsd")?.toFloat()
			totalWeeklyCostTable.add([todayDate.getTime(),currentCost])		        
		}        
	}
    
	state?.totalWeeklyDistanceTable = totalWeeklyDistanceTable
	state?.totalMonthlyDistanceTable = totalMonthlyDistanceTable
	state?.totalWeeklyCostTable = totalWeeklyCostTable
	state?.totalMonthlyCostTable = totalMonthlyCostTable
	state?.dailyDistanceTable = dailyDistanceTable
	state?.dailyCostTable = dailyCostTable
	if (settings.trace) {    
		log.debug ("dailyDistanceTable (size=${state?.dailyDistanceTable.size()}) =${state?.dailyDistanceTable}")  
		log.debug ("dailyCostTable (size=${state?.dailyCostTable.size()}) =${state?.dailyCostTable}")  
		log.debug ("totalWeeklyDistanceTable (size=${state?.totalWeeklyDistanceTable.size()}) =${state?.totalWeeklyDistanceTable}")  
		log.debug ("totalMonthlyDistanceTable (size=${state?.totalMonthlyDistanceTable.size()}) =${state?.totalMonthlyDistanceTable}")  
		log.debug ("totalWeeklyCostTable=  (size=${state?.totalWeeklyCostTable.size()}) ${state?.totalWeeklyCostTable}")  
		log.debug ("totalMonthlyCostTable= (size=${state?.totalMonthlyCostTable.size()}) ${state?.totalMonthlyCostTable}")  
	}        
}
...
String getDataString(Integer seriesIndex) {
	...
	switch (seriesIndex) {
		case 1:
			dataTable = state?.dailyDistanceTable
			break
		case 2:
			dataTable = state?.dailyCostTable
			break
		case 3:
			dataTable = state?.totalWeeklyDistanceTable
			break
		case 4:
			dataTable = state?.totalMonthlyDistanceTable
			break
		case 5:
			dataTable = state?.totalWeeklyCostTable
			break
		case 6:
			dataTable = state?.totalMonthlyCostTable
			break
	}
	dataTable.each() {
...
//	if (settings.trace) {    
//		log.debug("seriesIndex= $seriesIndex, dataString=$dataString")
//	}    
	return dataString
}
...
def getStartTime() {
	...
	if (state?.distanceTable?.size() > 0) {
		startTime = state.distanceTable.min{it[0].toLong()}[0].toLong()
	}
	if (state?.totalWeeklyCostTable.size() > 0) {
		startTime = Math.min(startTime, state.totalWeeklyCostTable.min{it[0].toLong()}[0].toLong())
	}
	if (settings.trace) {    
			...
			<html>
				<head>
					...
					function drawChart() {
						...
						data.addColumn('number', 'DailyDistance');
						data.addColumn('number', 'DailyCost');
						data.addColumn('number', 'WeeklyDist');
						data.addColumn('number', 'MonthlyDist');
						data.addColumn('number', 'WeeklyCost');
						data.addColumn('number', 'MonthlyCost');
						data.addRows([
							${dataRows}
						]);
						var options = {
							...
							series: {
								0: {targetAxisIndex: 1, color: '#44b621',lineWidth: 1},
								1: {targetAxisIndex: 0, color: '#FF0000', lineWidth: 1},
								2: {targetAxisIndex: 1, color: '#44b621',lineWidth: 2},
								3: {targetAxisIndex: 1, color: '#44b621',lineWidth: 3},
								4: {targetAxisIndex: 0, color: '#FF0000',lineWidth: 2},
								5: {targetAxisIndex: 0, color: '#FF0000',lineWidth: 3}
							},
							vAxes: {
								0: {
									title: 'Cost',
								...
								1: {
									title: 'Distance (${getDistanceScale()})',
									format: 'decimal',
	  		...
	  		<h3 style="font-size: 20px; font-weight: bold; text-align: center; background: #ffffff; color: #44b621;">DistanceVsCost</h3>
			<body>
