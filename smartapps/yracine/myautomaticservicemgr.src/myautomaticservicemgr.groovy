		...
		httpGet(deviceListParams) { resp ->
			...
			if (resp.status == AUTOMATIC_SUCCESS) {
				...
				jsonMap.results.each {
					...
					def dni = [ app.id, model, vehicleId].join('.')
					devices[dni] = model
...
def takeAction() {
	...
	def devices = AutomaticDevices.collect { dni ->
		...
		if ((atomicState?.exceptionCount>=MAX_EXCEPTION_COUNT) || ((exceptionCheck) && (exceptionCheck.contains("Unauthorized")))) {
			// need to re-authenticate again    
			atomicState.authToken= null                    
			atomicState?.oauthTokenProvided = false
			msg = "MyAutomaticServiceMgr>too many exceptions/errors or unauthorized exception, $exceptionCheck (${atomicState?.exceptionCount} errors), may need to re-authenticate at Automatic..." 
			log.error msg
			send msg
...
def swapToken() {
	...
	def authexptime = new Date((now() + (jsonMap.expires_in * 365 * 24 * 60 * 60 * 1000))).getTime() // the token is valid for a year
...
def getSmartThingsClientId() { "e8999651eb798fa8fec5" }

def getSmartThingsPrivateKey() { "2a6b55184631f9492e6559d72db5a2ae494eaecf" }